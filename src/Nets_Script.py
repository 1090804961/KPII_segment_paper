...
class ConvolutionalLayer(torch.jit.ScriptModule):
...
class conv_block(torch.jit.ScriptModule):
...
class up_conv(torch.jit.ScriptModule):
    def __init__(self, in_channels, out_channels):
...
class AttU_Net(torch.jit.ScriptModule):
...
class Attention_block(torch.jit.ScriptModule):
